...
const AdvancedTab = ({ showConfirmation }) => {
  ...
  const resetAllSettings = useCallback(() => {
    showConfirmation({
      ...
      message:
        "This will permanently delete ALL settings, blocked words, blocked sites, replacement phrases, and statistics. Your extension will be restored to its original state. This action CANNOT be undone!",
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateSettings({ scanInterval: parseInt(e.target.value) })
                }
                className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-primary"
              />
              <p className="text-xs text-gray-500 mt-1">
                How often to scan for new content (lower = more aggressive)
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateSettings({ mutationDebounce: parseInt(e.target.value) })
                }
                className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-primary"
              />
              <p className="text-xs text-gray-500 mt-1">
                Delay before processing DOM changes
              </p>
            </div>
          </div>
        </div>
